...
abstract class Controller_Rest extends \Controller
{
     ...
     */
    protected $_supported_formats = ['xml' => 'application/xml', 'rawxml' => 'application/xml', 'json' => 'application/json', 'jsonp' => 'text/javascript', 'serialized' => 'application/vnd.php.serialized', 'php' => 'text/plain', 'html' => 'text/html', 'csv' => 'application/csv'];
    ...
    protected function _detect_format()
    {
        ...
        if ($acceptable = \Input::server('HTTP_ACCEPT') and \Config::get('rest.ignore_http_accept') !== true) {
            ...
            $fragments = new \Caching_Iterator(new \ArrayIterator(preg_split('/[,;]/', $acceptable)));
            $acceptable = [];
            ...
            foreach ($fragments as $fragment) {
                ...
                if ($fragments->has_next()) {
                    $next = $fragments->get_inner_iterator()->current();
