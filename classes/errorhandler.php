 ...
 */
class Php_Error_Exception extends \ErrorException
{
...
{
    public static $loglevel = \Fuel::L_ERROR;
    public static $levels = [0 => 'Error', E_ERROR => 'Fatal Error', E_WARNING => 'Warning', E_PARSE => 'Parsing Error', E_NOTICE => 'Notice', E_CORE_ERROR => 'Core Error', E_CORE_WARNING => 'Core Warning', E_COMPILE_ERROR => 'Compile Error', E_COMPILE_WARNING => 'Compile Warning', E_USER_ERROR => 'User Error', E_USER_WARNING => 'User Warning', E_USER_NOTICE => 'User Notice', E_STRICT => 'Runtime Notice', E_RECOVERABLE_ERROR => 'Runtime Recoverable error', E_DEPRECATED => 'Runtime Deprecated code usage', E_USER_DEPRECATED => 'User Deprecated code usage'];
    ...
    public static function exception_handler($e)
    {
        ...
        if ($e instanceof \Exception or PHP_VERSION_ID >= 70000 and $e instanceof \Error) {
            ...
            $severity = !isset(static::$levels[$e->get_code()]) ? $e->get_code() : static::$levels[$e->get_code()];
            logger(static::$loglevel, $severity . ' - ' . $e->get_message() . ' in ' . $e->get_file() . ' on line ' . $e->get_line(), ['exception' => $e]);
            if (\Fuel::$env != \Fuel::PRODUCTION) {
     ...
     */
    public static function error_handler($severity, $message, $filepath, $line): bool
    {
        // don't do anything if error reporting is disabled
        if (error_reporting() !== 0) {
            $fatal = !in_array($severity, \Config::get('errors.continue_on', []));
            if ($fatal) {
                throw new \Php_Error_Exception($message, $severity, 0, $filepath, $line);
            }
            // non-fatal, recover from the error
            $e = new \Php_Error_Exception($message, $severity, 0, $filepath, $line);
            $e->recover();
        }
        return true;
    }
     ...
     */
    public static function notice(string $msg, $always_show = false): void
    {
    ...
    {
        $fatal = !in_array($e->get_code(), \Config::get('errors.continue_on', []));
        ...
        if (\Fuel::$is_cli) {
            ...
            if (\Config::get('cli_backtrace')) {
                ...
                \Cli::write(\Debug::backtrace($e->get_trace()));
            }
            if (!$fatal) {
                return;
            }
    ...
    protected static function prepare_exception($e, $fatal = true): array
    {
        $data = [];
        $data['type'] = $e::class;
        $data['severity'] = $e->get_code();
        $data['message'] = $e->get_message();
        $data['filepath'] = $e->get_file();
        $data['error_line'] = $e->get_line();
        $data['backtrace'] = $e->get_trace();
        // support for additional DB info
        if ($e instanceof \Database_Exception and $e->get_db_code()) {
            $data['severity'] .= ' (' . $e->get_db_code() . ')';
        }
        ...
        if ($e instanceof \Soap_Fault) {
            $data['soap']['faultcode'] = $e->faultcode;
            $data['soap']['faultstring'] = $e->faultstring;
            $data['soap']['errortype'] = empty($e->detail->exception_detail) ? '' : $e->detail->exception_detail->Type;
            $data['soap']['backtrace'] = empty($e->detail->exception_detail) ? '' : $e->detail->exception_detail->stack_trace;
