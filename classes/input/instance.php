...
class Input_Instance
{
    ...
    public function __construct(
         ...
         */
        protected ?\Fuel\Core\Request $request = null,
        Input_Instance $input = null
    )
    {
        // get php raw input
    ...
    public function _set($method, array $input): void
    {
        ...
        if ($method and property_exists($this, 'input_' . $method)) {
            ...
            throw new \Fuel_Exception('Input method "' . $method . '" is not defined.');
        }
    }
    /**
        ...
        if ($content_type == 'application/x-www-form-urlencoded') {
            ...
            if ($method === 'get' or $method === 'post') {
                if ($php_input and ($amps = substr_count($php_input, '&')) > ini_get('max_input_vars')) {
                    throw new \Php_Error_Exception('Input truncated by PHP. Number of variables exceeded ' . ini_get('max_input_vars') . '. To increase the limit to at least the ' . $amps . ' needed for this HTTP request, change the value of "max_input_vars" in php.ini.', 1, E_WARNING, __FILE__, __LINE__ + 7);
                }
            } else {
