...
class Log
{
    ...
    public static function initialize(): void
    {
        ...
        if (!is_dir($path) or !is_writable($path)) {
            ...
            throw new \Fuel_Exception('Unable to create the log file. The configured log path "' . $path . '" does not exist.');
        }
        ...
        try {
            ...
            \Config::set('log_threshold', \Fuel::L_NONE);
            throw new \Fuel_Exception('Unable to access the log file. Please check the permissions on ' . \Config::get('log_path') . '. (' . $e->get_message() . ')');
        }
        static::$path = $path;
        ...
        $stream = new \Monolog\Handler\Stream_Handler($path . $filename, \Monolog\Logger::DEBUG);
        $formatter = new \Monolog\Formatter\Line_Formatter('%level_name% - %datetime% --> %message%' . PHP_EOL, \Config::get('log_date_format', 'Y-m-d H:i:s'));
        $stream->set_formatter($formatter);
        static::$monolog->push_handler($stream);
    }
    /**
    ...
    protected static function need_logging($level)
    {
        ...
        static $levels = [100 => 'DEBUG', 200 => 'INFO', 250 => 'NOTICE', 300 => 'WARNING', 400 => 'ERROR', 500 => 'CRITICAL', 550 => 'ALERT', 600 => 'EMERGENCY'];
        // defined old default error labels
        static $oldlabels = [1 => 'Error', 2 => 'Warning', 3 => 'Debug', 4 => 'Info'];
        ...
        if (is_int($level) and !isset($levels[$level]) or is_string($level) and !array_search(strtoupper($level), $levels)) {
            throw new \Fuel_Exception('Invalid level "' . $level . '" passed to logger()');
        }
