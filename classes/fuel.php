...
class Fuel
{
    ...
    public static function init($config): void
    {
        ...
        try {
            ...
            throw new \Php_Error_Exception($e->get_message());
        }
        ...
        if (isset($array['classes'])) {
            foreach ($array['classes'] as $class) {
                if (!class_exists($class = \Str::ucwords($class))) {
                    throw new \Fuel_Exception('Class ' . $class . ' defined in your "always_load" config could not be loaded.');
                }
            }
        }
        /**
    ...
    public static function clean_path($path): string|array
    {
        ...
        static $paths = ['APPPATH/' => APPPATH, 'COREPATH/' => COREPATH, 'PKGPATH/' => PKGPATH, 'DOCROOT/' => DOCROOT, 'VENDORPATH/' => VENDORPATH];
