...
class Profiler
{
    ...
    {
        if (!static::$profiler) {
            static::$profiler = new \Php_Quick_Profiler(FUEL_START_TIME);
            static::$profiler->queries = [];
            static::$profiler->query_count = 0;
    ...
    {
        static::$profiler and \Console::log_speed($label);
    }
    public static function mark_memory($var = false, $name = 'PHP'): void
    {
        static::$profiler and \Console::log_memory($var, $name);
    }
    ...
    {
        if (static::$profiler) {
            static::$query = ['sql' => \Security::htmlentities($sql), 'time' => static::$profiler->get_micro_time(), 'stacktrace' => $stacktrace, 'dbname' => $dbname];
            return true;
        }
    }
    ...
    {
        if (static::$profiler) {
            static::$query['time'] = (static::$profiler->get_micro_time() - static::$query['time']) * 1000;
            static::$profiler->queries[] = static::$query;
            static::$profiler->query_count++;
        }
    }
    ...
    {
        return [microtime(true) - FUEL_START_TIME, memory_get_peak_usage() - FUEL_START_MEM];
    }
}