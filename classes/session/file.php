...
class Session_File extends \Session_Driver
{
        ...
        if (mt_rand(0, 100) < $this->config['gc_probability']) {
            if ($handle = opendir($this->config['path'])) {
                ...
                while (($file = readdir($handle)) !== false) {
                    if (filetype($this->config['path'] . $file) == 'file' and str_starts_with($file, $this->config['cookie_name'] . '_') and filemtime($this->config['path'] . $file) < $expire) {
    ...
    protected function _write_file($session_id, $payload)
    {
        ...
        if ($handle) {
            ...
            throw new \Fuel_Exception('Could not open the session file in "' . $this->config['path'] . ' for write access');
        }
        return $exists;
    }
    // --------------------------------------------------------------------
