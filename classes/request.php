    ...
    )
    {
        ...
        if (count($this->uri->get_segments()) and $module_path = \Module::exists($this->uri->get_segment(1))) {
            ...
            if (is_file($module_path .= 'config/routes.php')) {
                ...
                $reserve_routes = ['_root_' => $module, '_403_' => '_403_', '_404_' => '_404_', '_500_' => '_500_', $module => $module];
                $prepped_routes = [];
    ...
    public function execute($method_params = null): static
    {
        ...
        try {
            if ($this->route->callable !== null) {
                ...
                $this->controller_instance = $class->new_instance($this);
                $this->action = $this->action ?: ($class->has_property('default_action') ? $class->get_property('default_action')->get_value($this->controller_instance) : 'index');
                $method = $method_prefix . $this->action;
                // Allow to do in controller routing if method router(action, params) exists
                if ($class->has_method('router')) {
                    $method = 'router';
                ...
                if ($class->has_method($method)) {
                    $action = $class->get_method($method);
                    if (!$action->is_public()) {
                        throw new \Http_Not_Found_Exception();
                    }
                    if (count($this->method_params) < $action->get_number_of_required_parameters()) {
                        throw new \Http_Not_Found_Exception();
                    }
                    ...
                    $class->has_method('before') and $class->get_method('before')->invoke($this->controller_instance);
                    $response = $action->invoke_args($this->controller_instance, $this->method_params);
                    $class->has_method('after') and $response = $class->get_method('after')->invoke($this->controller_instance, $response);
        ...
        if ($response instanceof Response) {
            ...
            throw new \Fuel_Exception($this->controller_instance::class . '::' . $method . '() or the controller after() method must return a Response object.');
        }
