...
class File_Area
{
    ...
    public function get_handler($path, array $config = [], $content = [])
    {
        ...
        if (is_file($path)) {
            ...
            if (!empty($this->extensions) && !in_array($info['extension'], $this->extensions)) {
                throw new \File_Access_Exception('File operation not allowed: disallowed file extension.');
            }
        ...
        throw new \File_Access_Exception('Invalid path for file or directory.');
    }
    /**
    ...
    public function get_path($path): string
    {
        ...
        if (!empty($this->basedir) && !str_starts_with($pathinfo['dirname'], $this->basedir)) {
            throw new \Outside_Area_Exception('File operation not allowed: given path is outside the basedir for this area.');
        }
        // check file extension
        if (!empty($this->extensions) && array_key_exists($pathinfo['extension'], $this->extensions)) {
            throw new \File_Access_Exception('File operation not allowed: disallowed file extension.');
        }
