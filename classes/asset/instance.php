...
class Asset_Instance
{
     ...
     */
    protected $_asset_paths = ['css' => [], 'js' => [], 'img' => []];
    /**
     * @var  array  the sub-folders to be searched
     */
    protected $_path_folders = ['css' => 'css/', 'js' => 'js/', 'img' => 'img/'];
    /**
    ...
    protected function _parse_assets($type, $assets, $attr, $group, $raw = false)
    {
        ...
        foreach ($assets as $key => $asset) {
            ...
            $this->_groups[$group][] = ['type' => $type, 'file' => $asset, 'raw' => $raw, 'attr' => (array) $attr];
        }
    }
    // --------------------------------------------------------------------
    /**
